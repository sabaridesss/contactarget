...
$query = "update `news_tbl1` set `newspub_date` ='".addslashes($publish_date)."',
										`news_title` = '".addslashes($news_title)."',
										`short_desc` =  '".addslashes($short_desc)."',
										 `brief_desc` =  '".addslashes($brief_desc)."'
										 where `news_id` = '".$_REQUEST["news_id"]."'" ;
	$rs = mysql_query($query);

 	if($rs){
		//echo 'suceesfully added';
			header("location:view_news.php?msg=3");
		}
 
}

$edit_query = "select * from `news_tbl1` where news_id='".$_REQUEST["news_id"]."'";
$edit_rs = mysql_query($edit_query);
while($edit_row = mysql_fetch_array($edit_rs)){

	$newspub_date_edit = $edit_row['newspub_date'];
	$news_title_edit = $edit_row['news_title'];
	$short_desc_edit = $edit_row['short_desc'];
	$brief_desc_edit = $edit_row['brief_desc'];
...
<table width="980" border="0" cellpadding="5">
  <tr>
    ...
    <td width="61"><a href="news_add.php"><strong> News </strong></a></td>
  ...
  <tr>
    ...
    <td height="395" colspan="7" valign="top"><table width="825" border="0" align="center" cellpadding="5" bgcolor="#F5F5F5" >
      ...
      <tr>
        ...
        <td colspan="2" align="left"><input type="text" name="news_title" id="news_title"  value="<?=$news_title_edit?>"/></td>
      </tr>
      <tr>
        <td align="right" valign="top" id="title_name">Publish Date: </td>
        <td colspan="2" align="left">
		<input name="publish_date" type="text" id="publish_date" style="width:70px;" value="<?=$newspub_date_edit?>">
	...
	new tcal ({
      ...
      <tr>
        ...
        <td colspan="2" align="left"><textarea name="short_desc" cols="80" rows="5" id="short_desc"><?=$short_desc_edit?></textarea></td>
      </tr>
      <tr>
        <td align="right" valign="top" id="title_name">Brief - Description:</td>
        <td colspan="2" align="left"><textarea name="brief_desc" cols="80" rows="20" id="brief_desc"><?=$brief_desc_edit?></textarea></td>
      </tr>
      <tr>
