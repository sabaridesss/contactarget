...
//$rs = mysql_query($query);
  $query = "INSERT INTO menu_page_tbl(title,file_name,order_id,is_menu,parent_id)VALUES('".$sub_title."','".$page_name."','".$menu_order."','3','".$parent_id."')";

		$rs = mysql_query($query);
		if($rs){
		
			header("location:sub_menus1.php?parent_id=$parent_id&msg=add");
		}
}
...
$subcat_result = mysql_query($subcat_query);
	...
	function recursive($id,$link)
	{
		...
		if($par_id != 0) {
			$link=" >> <a href='sub_menus1.php?parent_id=".$id."'>".$title."</a>".$link;
			
			recursive($par_id,$link);
		} else {
			 
			 echo $link="<a href='main_page.php'>Main Menu</a>"." >> <a href='sub_menus1.php?parent_id=".$id."'>".$title."</a>".$link;
    ...
    <td align="left" valign="top" class="login-inner"><table width="100%" border="0" align="center" cellpadding="5" >
      ...
      <tr>
        ...
        <td align=""><?php if(isset($_SESSION['sadmin'])) {?> 
		 ...
		 <?php } ?>
		 &nbsp;&nbsp;&nbsp;<input type="button" name="Cancel" value="Cancel" class="addmenu2" onclick="return redirect_subEdit1('<?php echo $_REQUEST['subcat_id']; ?>','<?php echo $_REQUEST['parent_id']; ?>')"/>		 
		</td>
        </tr>
    </table></td>
  </tr>
