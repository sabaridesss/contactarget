...
function get_call_track_details1($start_date,$end_date,$pageno){
		
			$query = "select *,TIMEDIFF( created_date_time,created_date) as timespent,DATE_FORMAT(`created_date`,'%m/%d/%Y') AS date,track_date,campaign_list from `tracking_tbl` ";
			if($start_date != "" || $end_date != "")
			{
				$query .= "where";
				if($start_date != "" && $end_date == "")
				{
					$query .= "`created_date` LIKE '".addslashes($start_date)."%'";
				}
				else if($end_date != "" && $start_date == "")
				{
					...
					$query .= " `created_date` LIKE '".addslashes($end_date)."%'";
				}
				else if($start_date != "" && $end_date != "")
				{
					$query .= " `created_date` BETWEEN '".addslashes($start_date)."' AND '".addslashes($end_date)."'";
				}
...
function get_call_track_details($start_date,$end_date){
			
			
			$query = "select * from `tracking_tbl` ";
			if($start_date != "" || $end_date != "")
			{
				$query .= "where";
				if($start_date != "" && $end_date == "")
...
// -----starts-------- function for export option ----starts--------//
 ...
 // -----ends-------- function for export option ----ends--------//
  ...
  $num_rows  = get_num_record1($get_call_track_details_for_pagination);
   ...
   function get_page($limit,$pageno,$start_date,$end_date,$status,$check_unique)
{

	$get_call_track_details = get_call_track_details1($start_date,$end_date,$pageno);
...
$output_array = get_page($limit,$pageno,$start_date,$end_date,$status,$check_unique);
 ...
 if($num_rows > 20){
      if ($pageno == 1){
       ...
       {	
            ...
            $page_nation .=" <a class='menu_href' href='{$_SERVER['PHP_SELF']}?page=1&start_date=$start_date&end_date=$end_date'>FIRST</a>";
            $prevpage = $pageno-1;
            $page_nation .=" <a  class='menu_href' href='{$_SERVER['PHP_SELF']}?page=$prevpage&start_date=$start_date&end_date=$end_date'>PREV</a>";
       }
       ...
       {
		   $nextpage = $pageno+1;
           $page_nation .=" <a class='menu_href' href='{$_SERVER['PHP_SELF']}?page=$nextpage&start_date=$start_date&end_date=$end_date'>NEXT</a> ";
		   
           $page_nation .=" <a class='menu_href' href='{$_SERVER['PHP_SELF']}?page=$lastpage&start_date=$start_date&end_date=$end_date'>LAST</a> ";
       }
    }else if($num_rows<1){
...
<td height="40" colspan="26" align="left" >
...
&nbsp;&nbsp;<span style="color:#333; font-weight:bold;">End Date</span>&nbsp;&nbsp;<input type="text" name="end_date" id="end_date" value="<?php echo $end_date;?>" class="calender">
&nbsp;
...
<input type="submit" value="Export" class="addmenu2" name="Export"/></td>
      ...
      <tr bgcolor="#cccccc">
        ...
        <td align="left" class="table1">Keyword List</td>
	  ...
	  if(count($output_array)>0) {
        ...
        <td align="left" class="style3"><a onclick="TINY.box.show({url:'keyword_list.php',post:'id=<?php echo $id; ?>',boxid:'frameless',fixed:false,closejs:function(){closeJS()}})">List</a></td>
	 ...
	 $query_curl =  "select a.camp_id,b.c_name,b.c_keyword from compaign_name a, campaign_list b where a.company_admin='$company_admin' and b.id = a.camp_id  AND a.id = ".$output_array[$i]['28'];
