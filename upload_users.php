...
/*	$SMTP_Validator = new SMTP_validateEmail();
	...
	if ($results[$email1]==1) {*/
/*	$exsit      = "select email from user_tbl where company_admin='".$company_admin."' and email='".$email."'";
	$exsit_mail = mysql_query($exsit);
	if(mysql_num_rows($exsit_mail)==0)
	{*/
	if($count_usertype==0)
  	{
	...
	if($firstname != "" && eregi($pattern, $email))
	{
...
$query = "insert into user_tbl(company_admin,firstname, lastname, email, group_generl,user_type) values('".$company_admin."','".$firstname."','".$lastname."','".$email."','".$group_generl."','".$sort_order[$i]."')";
	...
	{
	$exsit_bounce      = "select email from bounce_tbl where company_admin='".$company_admin."' and email='".$email."'";
	$exsit_bounce_mail = mysql_query($exsit_bounce);
	if(mysql_num_rows($exsit_bounce_mail)==0)
	{
$query = "insert into bounce_tbl(company_admin,firstname,lastname, email, group_generl,user_type) values('".$company_admin."','".$firstname."','".$lastname."','".$email."','".$group_generl."','".$user_type."')";
$rs = mysql_query($query);
	}
	}*/
} 
