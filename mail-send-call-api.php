...
/* $pdffilename     		 =     "pdf/".$pdfname;
 ...
 $address_query = "select * from footer_content where company_admin=".$company_admin." order by id DESC";
$rs_address = mysql_query($address_query);
if(mysql_num_rows($rs_address)>0)
{
$item_address=mysql_fetch_array($rs_address);

if(trim($item_address['company'])!="")
{
...
if(trim($item_address['zipcode'])!="")
{
$zipcode=" | ".$item_address['zipcode'];
}
else
{
$zipcode="";
...
$information=$item_address['information'];
if(trim($information)!="")
{

$pp_link=' | <a style="color:#FD600B;text-decoration:none;"  href="http://www.mailtides.com/privacy-policy.php?sendlist='.$mailid.'&company_users='.$company_admin.'&campid='.$item_save1['camp_id'].'">Privacy Policy</a>';
...
$id=0;
 ...
 $mailer_address = $pp_link.$company.$address.$city.$state.$zipcode;
...
$template_midfooter=$crypt_url.'if this email does not display correctly';
$template_unsub='This email sent to '.$array_k['email'].' by '.$fromaddress.$mailer_address.' | <a target="_blank" style="color:#FD600B;text-decoration:none;"  href="http://www.mailtides.com/manage.php?user='.strtr(base64_encode(addslashes(gzcompress(serialize($array_k['id']),9))), '+/=', '-_,').'">Unsubscribe</a>';
...
$send = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><meta http-equiv="Content-Type" content="text/html;charset=UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>'.$subject.'</title></head><body>'.$template_url.$final_content.$template_url_end.'<img src="http://www.mailtides.com/get_count1.php?sent_id='. $array_k['id'].'&sendlist='.$mailid.'" alt="" title=""  width="1px" height="1px" style="display:none" /></table><table border="0" cellspacing="0" cellpadding="0" style="font-family:Helvetica,Arial,sans-serif;font-size:11px;font-family:Helvetica,Arial,sans-serif;color:#000000" width="100%"><tbody><tr><td>'.$template_midfooter.'</td></tr><tr><td>'.$template_unsub.'</td></tr><tr><td><table width="1" border="0" cellspacing="0" cellpadding="1"><tbody><tr><td><div style="min-height:10px;font-size:10px;line-height:10px">&nbsp;</div></td></tr></tbody></table></td></tr></tbody></table></body></html>';
