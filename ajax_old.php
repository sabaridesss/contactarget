...
$sel_query = "select * from `page_contents` where `parent_id` = '$menus_id'";
...
if($num_of_rows == 0)
{
	$query = "delete from main_category_list where Main_category_ID='".$menus_id."'";
	if(mysql_query($query))
	{
	 	echo 'Suceessfully deleted';
	}
...
if($deletetab_id != ""){
	...
	$query = "delete from taps_tpl where tap_id ='".$deletetab_id."'";
	if(mysql_query($query))
	{
	 	echo 'Suceessfully deleted';
	}
