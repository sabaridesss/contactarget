...
echo $bounces                = "select email from bounce_tbl where email='".$email."'";
...
$data_string = "http://ws.zigzan.com/ZigzanService.svc/VerifyEmail?email=".$email;  // build API request
...
{ 
  if($count_usertype==0)
  {
     ...
     } echo $insert = 'INSERT INTO bounce_tbl 
										SET
...
if(isset($_REQUEST['Update']))
	{ 
	 ...
	 $hid	= $_REQUEST['hid'];
	 echo $del_user_type=mysql_query("DELETE FROM `user_tbl` WHERE company_admin  = $company_admin and  `id` =$hid");
	        $firstname	= $_REQUEST['firstname'];
			$lastname	= $_REQUEST['lastname'];
...
<?php if($_REQUEST['id'] != '') { ?>
                     <input type="submit" name="Update" value="Update" class="btn btn-large btn-primary" onclick="return formValidator()" />  
                       ...
                       <input type="submit" name="Add" value="Add" class="btn btn-large btn-primary" onclick="return formValidator()" />   
                        <?php }?>
                         <input type="submit" name="Close" value="Cancel" class="btn btn-large btn-primary" /> </td>
                    </tr>
                  </table></td>
              </tr>
            </table>
